...
let lgaList = document.querySelector('#lga');
...
let displayCities = async () => {
    ...
    let c = lg;
        ...
        c.forEach(x =>{
        if (`${x.state} State` == getState.value || `${x.state}` == getState.value) {
            x.lgas.forEach(r => {
                
                getCities.innerHTML += `<option >${r}</option>`;
                
            })}})}

getCities.addEventListener("click", displayCities);
lgaList.addEventListener('change', (e) => {
    e.preventDefault();
    var lga = lgaList.options[lgaList.selectedIndex].value;
    lgaList.options[0].text = lga;
    console.log(lga);
})
